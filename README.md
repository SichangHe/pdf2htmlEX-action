# pdf2htmlEX GitHub Action

Convert PDF to
HTML using
[pdf2htmlEX](https://github.com/pdf2htmlEX/pdf2htmlEX?tab=readme-ov-file)
in GitHub Actions.

This action runs
[the pdf2htmlEX Docker image from
DockerHub](https://hub.docker.com/r/pdf2htmlex/pdf2htmlex/tags)
with the supplied arguments as is.

```yml
- uses: SichangHe/pdf2htmlEX-action@v0.0
  with:
    arguments: input.pdf output.html
```

The above is equivalent to the command below.
Note that the input PDF must be within the current working directory for
the container to be able to read it,
and the output is also at the current directory.

```sh
pdf2htmlEX input.pdf output.html
```

pdf2htmlEX takes these options:
